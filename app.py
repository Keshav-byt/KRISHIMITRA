...
def home():
    ...
    <body>
        ...
        <h2>Soil Analysis</h2>
        <form action="/soil-analysis-ui" method="post">
            N <input name="N"><br>
            P <input name="P"><br>
            K <input name="K"><br>
            pH <input name="pH"><br>
        ...
        <form action="/predict-crop-ui" method="post">
            Soil Color <input name="soil_color"><br>
...
@app.route("/soil-analysis-ui", methods=["POST"])
def soil_analysis_ui():
    ...
    scaled = SCALERS['soil'].transform(arr)
    pred = MODELS['soil'].predict(scaled)

    result = "High" if pred[0][0] > 0.5 else "Low"

    return f"<h2>Soil Fertility: {result}</h2><a href='/'>Go Back</a>"


@app.route("/predict-crop-ui", methods=["POST"])
def predict_crop_ui():
    data = request.form.to_dict()
    ...
    encoder = MODELS['soil_colour_enc']
    encoded = encoder.transform([soil_color])[0]

    temp, hum = get_weather(city)

    input_data = np.array([[encoded, temp, hum]])
    scaled = SCALERS['irrigation'].transform(input_data)

    pred = MODELS['irrigation'].predict(scaled)
    crop = LABEL_ENC.inverse_transform(pred)[0]
